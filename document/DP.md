...
| 章节                                           | 内容                   | 行数 |
| ---------------------------------------------- | ---------------------- | ---- |
| [产品概述](#产品概述)                          | 系统简介、核心功能     | ~30  |
| [技术栈](#技术栈)                              | 技术选型表             | ~20  |
| [项目架构](#项目架构)                          | 架构说明、模块划分     | ~30  |
| [TypeScript 配置](#typescript-配置)            | TS配置文件             | ~30  |
| [package.json](#packagejson)                   | 完整依赖配置           | ~40  |
| [开发环境变量](#开发环境变量)                  | 环境变量配置           | ~15  |
| [代理地址](#代理地址)                          | Vite代理配置表         | ~10  |
| [vite-config](#vite-config)                    | 完整构建配置           | ~200 |
| [APP.vue](#appvue)                             | 应用入口组件           | ~30  |
| [layouts](#layouts)                            | 布局组件               | ~50  |
| [Pinia配置](#pinia-配置)                       | 状态管理配置           | ~30  |
| [turf地理空间分析引擎](#turf-地理空间分析引擎) | 地理分析工具           | ~60  |
| [系统设置钩子](#系统设置钩子)                  | 页面加载状态           | ~30  |
| [Cesium配置](#cesium-配置)                     | 地图初始化、聚类、标签 | ~500 |
| [Cesium视图](#cesium-views)                    | 地图组件、侧边栏       | ~400 |
| [Cesium右键菜单组件](#cesium-右键菜单组件)     | 右键菜单Vue组件        | ~200 |
| [index.html](#indexhtml)                       | HTML入口               | ~25  |
| [Cookie工具](#cookie工具)                      | Cookie封装             | ~100 |
| [SSO SDK集成](#sso-sdk集成)                    | 单点登录               | ~80  |
| [指令](#指令)                                  | 加载指令               | ~200 |
| [Cesium 弹窗组件](#cesium-弹窗组件)            | Cesium弹窗             | ~200 |
| [路由设计](#路由设计)                          | 路由配置、守卫         | ~100 |
| [左侧筛选面板组件](#左侧筛选面板组件)          | 筛选面板               | ~350 |
| [is工具](#is-工具)                             | 类型判断工具           | ~90  |
| [MQTT配置](#mqtt配置)                          | 消息通信               | ~700 |
| [矿井数据](#矿井数据)                          | 数据结构示例           | ~150 |
| [类型定义](#类型定义)                          | TypeScript类型定义     | ~100 |
| [右键菜单绑定方法](#右键菜单绑定方法)          | 点击事件处理           | ~50  |
...
declare module 'pako' {
  export function ungzip(data: Uint8Array | Buffer | number[], options?: { to?: string }): string | Uint8Array
  export function gzip(data: string | Uint8Array, options?: object): Uint8Array
}

interface Window {
  BW_SSO_SDK: {
    SSOLogin: (hostname: string, port: number, callback: (data: any) => void) => void
  }
}
```
...
interface Cookies {
  (data?: string | CookieData, opt?: CookiesOptions): any;
  get(key: string): any;
